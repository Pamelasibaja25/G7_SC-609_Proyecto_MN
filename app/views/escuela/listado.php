...
<div class="container py-4">
    <h1 class="mb-4">Listado de Escuelas</h1>
        ...
        <table class="table table-striped table-bordered table-hover">
            ...
            <tbody>
                <?php if (empty($escuelas)): ?>
                    <tr>
                        <td colspan="4" class="text-center">No hay escuelas registradas.</td>
                    </tr>
                <?php else: ?>
                    <?php foreach ($escuelas as $e): ?>
                        <tr>
                            <td><?= htmlspecialchars($e['_id']) ?></td>
                            <td><?= htmlspecialchars($e['nombre']) ?></td>
                            <td><?= htmlspecialchars($e['provincia'] ?? '') ?></td>
                            <td>
                                <a href="editar.php?id_escuela=<?= urlencode($e['_id']) ?>"
                                   class="btn btn-sm btn-primary mb-1">
                                    Editar
                                </a>

                                <form method="post"
                                      action="../../controller/escuelaController.php"
                                      class="d-inline">
                                    <input type="hidden" name="action" value="eliminar-escuela">
                                    <input type="hidden" name="id_escuela" value="<?= htmlspecialchars($e['_id']) ?>">
                                    <button type="submit"
